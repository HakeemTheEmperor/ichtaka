...
def add_comment(db: Session, user: User_Account, post_id: int, data: CommentCreate):
...
def cast_vote(db: Session, user: User_Account, post_id: int, vote_type: VoteType):
    post = db.query(Post).filter(Post.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
def update_post_status(db: Session, post_id: int, new_status: PostStatus):
    post = db.query(Post).filter(Post.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
